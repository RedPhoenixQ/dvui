...
pub const Color = extern struct {
    ...
    // Assert that Color can be cast to [4]u8 rgba array
    // Casting a slice that is not a multiple of @sizeOf(Color) panics at runtime
    comptime {
        std.testing.expectEqual(@sizeOf(Color), 4) catch unreachable;
        const pixels = [12]u8{
            0x1, 0x2, 0x3, 0x4,
            0x5, 0x6, 0x7, 0x8,
            0x9, 0xA, 0xB, 0xC,
        ...
        const colors = &[3]Color{
            .{ .r = 0x1, .g = 0x2, .b = 0x3, .a = 0x4 },
            .{ .r = 0x5, .g = 0x6, .b = 0x7, .a = 0x8 },
            .{ .r = 0x9, .g = 0xA, .b = 0xB, .a = 0xC },
        };
        const color_bytes: []const u8 = @ptrCast(colors);
        std.testing.expectEqual(color_bytes.len, pixels.len) catch unreachable;
        std.testing.expectEqualSlices(u8, color_bytes, &pixels) catch unreachable;
    }
    ...
    pub fn asBytes(self: Color) [4]u8 {
        return @bitCast(self);
    }
    ...
    pub fn fromTheme(theme_color: dvui.Options.ColorsFromTheme) Color {
    ...
    pub const PMA = extern struct {
        ...
        // Assert that PMA can be cast to [4]u8 rgba array
        // Casting a slice that is not a multiple of @sizeOf(PMA) panics at runtime
        comptime {
            std.testing.expectEqual(@sizeOf(PMA), 4) catch unreachable;
            const pixels = [12]u8{
                0x1, 0x2, 0x3, 0x4,
                0x5, 0x6, 0x7, 0x8,
                0x9, 0xA, 0xB, 0xC,
        ...
        /// Convert normal color to premultiplied alpha.
        pub fn fromColors(colors: []Color) []PMA {
            const pma: []PMA = @ptrCast(colors);
            for (pma) |*c| {
                c.* = PMA.fromColor(c.castToColor());
            }
        }
